...
> 下一代超高性能 JSON Schema 校验"物理加速卡"

IRIS 的目标：在现代 CPU 上把 JSON Schema 验证的吞吐推到纯解析库（simdjson）的 5 倍以内，
方法是彻底抛弃"解析-建树-遍历"二次伤害架构，改走 **Fused Parse & Validate** 单遍扫描 + SIMD 凌空匹配 + Bitwise DFA + AOT 完美哈希。
...
`$dynamicRef`、pattern (**RE2** 后端，线性时间 ReDoS-safe)、annotation tracking 的
`unevaluatedProperties` / `unevaluatedItems`。Fast Path 编译失败的 schema 自动落
到 Slow Path，**永不假阳性通过**。详见 [PERFORMANCE.md §12](./PERFORMANCE.md#12-慢车道slow-path真实现)。
...
| 慢车道解释器 | `src/slow_path.cpp` | 🟡 stub（暂复用 Fast Path） |
...
- **Phase 3：JIT 动态指令生成** — ✅ asmjit trampoline 验收；🟡 schema-specialized codegen 下阶段
- **Phase 4：工业级降级 & Bowtie 合规** — ✅ Schema 解析 / ajv & simdjson 横向 / Bowtie driver；🟡 慢车道 QuickJS 待补

## 6. 下一步计划

1. **JIT v0.2**：把 trampoline 升级为按 schema 烧入立即数的 specialized codegen
2. **Bowtie OCI 发布**：将 `bowtie_iris` 打包推送到 `bowtie-json-schema` 仓库
3. **慢车道补全**：`$ref` / `allOf` / `pattern` interpreter，目标 Bowtie 100% 绿灯
4. **Apple Instruments 集成**：CI 自动跑 `xctrace --template 'CPU Counters'`，观察 Cache Miss / Branch Misprediction
